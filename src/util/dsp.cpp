                                         ...
                                         double ratio, int N)
{
    ...
    // speed > 1.0 => faster => Hs larger
    // speed < 1.0 => slower => Hs smaller
    const int Hs = std::max(1, int(std::round(Ha * speed)));
    ...
    for (size_t pos = 0; pos + N <= frames; pos += Ha) {
        ...
        // phase propagation (unchanged from pitch shifter)
        for (int k = 0; k < outBins; ++k) {
            double omega = 2.0 * M_PI * k / N;
            double expected = omega * Ha;
            double delta = princarg(phase[k] - prevPhase[k] - expected);
